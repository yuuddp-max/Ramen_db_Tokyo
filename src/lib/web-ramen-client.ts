    ...
    async searchRecentMentions() {
      ...
      const response = await fetch("https://api.openai.com/v1/responses", {
        ...
        body: JSON.stringify({
          ...
          input: `あなたは東京ラーメンのWeb調査担当です。現在時刻は${now.toISOString()}です。Web検索を使い、過去7日間（${since}以降）の東京のラーメン情報を調査してください。値引き、限定メニュー、テレビ・ニュース掲載、行列、SNSで話題になっている可能性がある店舗を優先し、話題候補を最大20件選んでください。Xを含む公開Web上の情報を対象にしてください。各候補について、店名、エリア、話題の理由、情報の日付、根拠（直接URL）をまとめてください。カップ麺、インスタント麺、自宅調理、東京以外の記事、ラーメンと無関係なページは除外してください。同じURLは1件だけにしてください。本文の長い転載はせず、summaryは80〜500文字の要約にしてください。source_scoreは新しさ、東京との関連、話題性、店舗情報の具体性、信頼できる一次情報を0〜100で評価してください。published_atが確認できない場合はnullにしてください。URLはHTTPSの直接URLのみ返してください。`,
