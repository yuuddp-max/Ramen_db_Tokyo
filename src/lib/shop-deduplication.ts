...
export function dedupeRamenShops(shops: RamenShop[]) {
  ...
  return shops.filter((shop) => {
    const name = normalizeShopText(shop.name);
    const address = normalizeShopText(shop.address);
    const location = `${shop.latitude.toFixed(4)},${shop.longitude.toFixed(4)}`;
    const key = name && address ? `store:${name}:${address}` : `place:${normalizeShopText(shop.place_id) || `${name}:${location}`}`;
