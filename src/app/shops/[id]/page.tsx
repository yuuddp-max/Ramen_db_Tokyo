...
import { formatPriceLevel, formatStatus, getCurrentOpenStatus, inferRamenStyle } from "@/lib/utils";
...
export default async function ShopDetail({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [station] = await Promise.all([getNearestStation(shop)]);
  ...
  return <main className="min-h-screen"><RecentShopTracker shopId={shop.id} shopName={shop.name} /><header className="border-b border-white/10"><div className="mx-auto flex max-w-6xl items-center justify-between px-5 py-4 sm:px-8"><Link href="/" className="font-black">TOKYO <span className="text-ramen">RAMEN</span></Link><Link href="/" className="text-sm text-stone-400 hover:text-gold">← 一覧へ戻る</Link></div></header>
    <div className="mx-auto max-w-6xl px-5 py-9 sm:px-8 sm:py-14"><p className="text-xs font-bold tracking-[.25em] text-gold">RAMEN SHOP</p><div className="mt-3 flex flex-col justify-between gap-4 sm:flex-row sm:items-end"><div><h1 className="text-3xl font-black sm:text-5xl">{shop.name}</h1><p className="mt-3 text-sm text-stone-400">{shop.address}</p></div><div className="flex flex-wrap gap-2"><a className="rounded-full border border-gold bg-gold px-4 py-2 text-sm font-bold text-ink transition hover:bg-transparent hover:text-gold" href={`https://www.google.com/maps/dir/?api=1&destination=${shop.latitude},${shop.longitude}&destination_place_id=${shop.place_id}`} target="_blank" rel="noreferrer">⌖ 経路</a><a className="rounded-full border border-white/20 px-4 py-2 text-sm font-bold text-stone-200 transition hover:border-gold hover:text-gold" href={googleMapsUrl} target="_blank" rel="noreferrer">Google Maps ↗</a><ShareButton shopId={shop.id} shopName={shop.name} /><FavoriteButton shopId={shop.id} /></div></div>
      <div className="mt-8 grid gap-5 lg:grid-cols-[1.3fr_.7fr]"><div className="space-y-5">{shop.photo_name && <figure className="panel overflow-hidden rounded-2xl"><Image unoptimized src={`/api/shop-photo?shopId=${shop.id}`} alt={`${shop.name} の店舗写真`} width={1200} height={800} className="h-72 w-full object-cover sm:h-96" /><figcaption className="border-t border-white/10 px-4 py-2 text-xs text-stone-500">写真提供：{shop.photo_attributions?.length ? shop.photo_attributions.map((attribution, index) => <span key={`${attribution.displayName}-${index}`}>{index > 0 && "、"}{attribution.uri ? <a className="underline hover:text-gold" href={attribution.uri} target="_blank" rel="noreferrer">{attribution.displayName ?? "Googleユーザー"}</a> : attribution.displayName ?? "Googleユーザー"}</span>) : "Google"}</figcaption></figure>}<MapView shops={[shop]} selected={shop} className="h-[380px] overflow-hidden rounded-2xl border border-white/10" /></div><div className="panel rounded-2xl p-6"><p className="text-sm text-stone-400">Google の評価</p><p className="mt-1 text-4xl font-black text-gold">★ {shop.rating?.toFixed(1) ?? "–"}</p><p className="mt-1 text-sm text-stone-500">{shop.user_ratings_total?.toLocaleString() ?? 0} 件の口コミ</p><div className="mt-6 border-t border-white/10 pt-5 text-sm"><p><span className="text-stone-500">いまの営業時間</span><span className={`float-right font-bold ${openStatus.open ? "text-emerald-400" : openStatus.known ? "text-stone-300" : "text-stone-500"}`}>{openStatus.label}</span></p><p className="mt-4"><span className="text-stone-500">価格帯</span><span className="float-right">{formatPriceLevel(shop.price_level)}</span></p><p className="mt-4"><span className="text-stone-500">Google登録状態</span><span className="float-right text-stone-300">{formatStatus(shop.business_status)}</span></p></div></div></div>
      <section className="panel mt-5 rounded-2xl p-6"><h2 className="font-bold">訪問の目安</h2><div className="mt-5 grid gap-5 sm:grid-cols-2 lg:grid-cols-4"><div><p className="text-xs text-stone-500">ラーメンのジャンル</p><p className="mt-1 font-bold text-gold">{inferRamenStyle(shop.name)}</p></div><div><p className="text-xs text-stone-500">営業開始・終了（本日）</p><p className="mt-1 font-bold">{todayHours?.opensAt && todayHours.closesAt ? `${todayHours.opensAt} 〜 ${todayHours.closesAt}` : "情報なし"}</p></div><div><p className="text-xs text-stone-500">現在の推定混雑度</p><p className="mt-1 font-bold">{visitEstimate.crowd}<span className="ml-2 text-xs font-normal text-stone-500">待ち時間 {visitEstimate.wait}</span></p></div><div><p className="text-xs text-stone-500">最寄駅からの距離</p><p className="mt-1 font-bold">{station ? `${station.name} 約${station.distanceM.toLocaleString()}m` : "取得中・情報なし"}</p></div></div><p className="mt-5 text-xs leading-5 text-stone-500">混雑度・待ち時間は、現在時刻・評価件数をもとにした参考値です。実際の状況とは異なる場合があります。</p></section>
