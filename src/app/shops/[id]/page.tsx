...
export default async function ShopDetail({ params }: { params: Promise<{ id: string }> }) {
  ...
  const soupType = shop.researched_soup_type ?? ramen.soup;
  const ramenStyle = shop.researched_style ?? ramen.style;
  const researchConfidence = shop.research_confidence === "high" ? "高" : shop.research_confidence === "medium" ? "中" : shop.research_confidence === "low" ? "低" : null;
