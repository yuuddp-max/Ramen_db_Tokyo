...
export async function GET(request: NextRequest) {
  ...
  const examples = new Map((data ?? []).map((row) => [`${row.shop_id ?? "legacy"}:${row.source_hash}:${row.soup_category}:${row.style_category}`, row]));
  for (const shop of manualShops ?? []) {
    ...
    for (const [key, existing] of examples) if (existing.shop_id === shop.id) examples.delete(key);
    const row = { shop_id: shop.id, classification_text: classificationText, source_hash: sourceHash, soup_category: shop.soupCategory, style_category: shop.styleCategory, created_at: shop.classifiedAt ?? new Date().toISOString() };
    examples.set(`manual:${shop.id}`, row);
  }
  const rows = ["classification_text,source_hash,soup_category,style_category,created_at", ...[...examples.values()].sort((a, b) => Date.parse(b.created_at) - Date.parse(a.created_at)).map((row) => [csv(row.classification_text), csv(row.source_hash), csv(row.soup_category), csv(row.style_category), csv(row.created_at)].join(","))];
