...
export default async function ResearchAdminPage() {
  ...
  if (authenticated && supabaseAdmin) {
    const [classificationReviewResult, totalResult, missingRatingResult, missingWebsiteResult, missingPhotoResult, classificationProcessed, classificationAutoApproved, classificationNeedsReview, classificationAi, classificationError, xFetchLogResult] = await Promise.all([
    ...
    metrics = {
      total: totalResult.count ?? 0, pending: 0, draft: 0,
      approved: 0, rejected: 0, missingRating: missingRatingResult.count ?? 0,
