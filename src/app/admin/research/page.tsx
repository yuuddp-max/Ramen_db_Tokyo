...
export default async function ResearchAdminPage() {
  ...
  if (authenticated && supabaseAdmin) {
    const [draftResult, totalResult, pendingResult, draftCountResult, approvedResult, rejectedResult, missingRatingResult, missingWebsiteResult, missingPhotoResult, classificationProcessed, classificationAutoApproved, classificationNeedsReview, classificationAi, classificationError, xFetchLogResult] = await Promise.all([
      supabaseAdmin.from("ramen_shops").select('place_id,name,address,rating,user_ratings_total,research_evidence_summary,"soupCategory","styleCategory","soupConfidence","styleConfidence","classificationMethod","classificationStatus"').eq("classificationStatus", "needs-review").order("classifiedAt", { ascending: false }).limit(30),
      ...
      supabaseAdmin.from("ramen_shops").select("id", { count: "exact", head: true }).eq("research_status", "draft"),
      supabaseAdmin.from("ramen_shops").select("id", { count: "exact", head: true }).eq("research_status", "approved"),
      supabaseAdmin.from("ramen_shops").select("id", { count: "exact", head: true }).eq("research_status", "rejected"),
      ...
      supabaseAdmin.from("ramen_shops").select("id", { count: "exact", head: true }).not("classificationStatus", "is", null),
      supabaseAdmin.from("ramen_shops").select("id", { count: "exact", head: true }).eq("classificationStatus", "auto-approved"),
      supabaseAdmin.from("ramen_shops").select("id", { count: "exact", head: true }).eq("classificationStatus", "needs-review"),
      supabaseAdmin.from("ramen_shops").select("id", { count: "exact", head: true }).eq("classificationMethod", "generative-ai"),
      supabaseAdmin.from("ramen_shops").select("id", { count: "exact", head: true }).eq("classificationStatus", "error"),
