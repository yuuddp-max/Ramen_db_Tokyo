...
function readState() {
  ...
  return { mode: (params.get("mode") === "news" ? "news" : "shops") as Mode, category: params.get("category") ?? "all", soup: params.get("soup") ?? "", style: params.get("style") ?? "", area: params.get("area") ?? "all", openOnly: params.get("open") === "1" };
}
...
export function RamenHomeShell({ initialShops, initialTotal, posts }: { initialShops: RamenShop[]; initialTotal: number; posts: WebRamenMentionWithShop[] }) {
  ...
  useEffect(() => { const sync = () => { const state = readState(); setMode(state.mode); setCategory(state.category); setSoup(state.soup); setStyle(state.style); setArea(state.area); setOpenOnly(state.openOnly); }; sync(); window.addEventListener("popstate", sync); return () => window.removeEventListener("popstate", sync); }, []);
  ...
  const shopKey = `${mode}-${soup}-${style}-${area}-${openOnly}`;
  const activeGenreLabel = useMemo(() => [soup && SHOP_GENRES.find((tab) => tab.id === `soup:${soup}`)?.label, style && SHOP_STYLES.find((tab) => tab.id === `style:${style}`)?.label].filter(Boolean).join("・") || "すべて", [soup, style]);
  return <div className="min-h-screen pb-16 md:pb-0">
    ...
    <main className="mx-auto max-w-[1200px] px-4 pb-10 sm:px-6"><div className="mt-4 rounded-2xl border border-border bg-white p-2 shadow-warm sm:mt-6"><div className="grid grid-cols-2 gap-2" role="tablist" aria-label="表示モード"><button type="button" role="tab" aria-selected={mode === "news"} onClick={() => changeMode("news")} className={`min-h-14 rounded-xl text-sm font-black transition duration-200 ${mode === "news" ? "bg-accent text-white" : "text-text-secondary hover:bg-background-subtle"}`}>◉ ニュース</button><button type="button" role="tab" aria-selected={mode === "shops"} onClick={() => changeMode("shops")} className={`min-h-14 rounded-xl text-sm font-black transition duration-200 ${mode === "shops" ? "bg-accent text-white" : "text-text-secondary hover:bg-background-subtle"}`}>🍜 ラーメン店</button></div></div>
      {mode === "news" ? <><div className="mt-5"><HorizontalTabs tabs={NEWS_CATEGORIES} value={category} onChange={changeCategory} label="ニュースカテゴリ" /></div><section className="mt-8"><p className="text-xs font-bold tracking-[.18em] text-accent">RAMEN NEWS</p><h1 className="mt-2 text-3xl font-black tracking-tight text-ink sm:text-4xl">東京ラーメンの今を読む</h1><p className="mt-3 text-sm leading-6 text-text-secondary">新店、限定メニュー、メディア掲載など、直近の東京ラーメン情報をまとめています。</p></section><FeaturedNewsCarousel posts={posts} /><NewsList posts={posts} category={category} /></> : <><div className="mt-5"><p className="mb-2 text-xs font-bold text-text-secondary">スープ系統</p><HorizontalTabs tabs={SHOP_GENRES} value={soup ? `soup:${soup}` : "all"} onChange={changeSoup} label="ラーメンのスープ系統" /></div><div className="mt-4"><p className="mb-2 text-xs font-bold text-text-secondary">スタイル</p><HorizontalTabs tabs={SHOP_STYLES} value={style ? `style:${style}` : "all"} onChange={changeStyle} label="ラーメンのスタイル" /></div><div className="mt-4"><p className="mb-2 text-xs font-bold text-text-secondary">地域</p><HorizontalTabs tabs={[{ id: "all", label: "すべて" }, ...AREAS]} value={area} onChange={changeArea} label="地域" /><button type="button" aria-pressed={openOnly} onClick={toggleOpenOnly} className={`mt-3 min-h-11 rounded-full border px-4 text-sm font-bold transition ${openOnly ? "border-accent bg-accent text-white" : "border-border bg-white text-text-secondary hover:border-accent hover:text-accent"}`}>営業中</button></div><div className="mt-7"><p className="text-xs font-bold tracking-[.18em] text-accent">RAMEN SHOPS</p><h1 className="mt-2 text-2xl font-black text-ink sm:text-3xl">今日の一杯を探す</h1><p className="mt-2 text-sm text-text-secondary">{activeGenreLabel}{area !== "all" ? `・${area}` : ""}から、気になるラーメン店を探せます。</p></div><div className="mt-4"><SearchExperience key={shopKey} initialShops={initialShops} initialTotal={initialTotal} /></div></>}</main>
