...
import { RAMEN_SOUPS } from "@/lib/ramen-genres";
...
function readState() {
  ...
  return { mode: (params.get("mode") === "news" ? "news" : "shops") as Mode, category: params.get("category") ?? "all", genre: params.get("soup") ? `soup:${params.get("soup")}` : params.get("style") ? `style:${params.get("style")}` : "all", area: params.get("area") ?? "all", openOnly: params.get("open") === "1" };
}
...
export function RamenHomeShell({ initialShops, initialTotal, posts }: { initialShops: RamenShop[]; initialTotal: number; posts: WebRamenMentionWithShop[] }) {
  ...
  useEffect(() => { const sync = () => { const state = readState(); setMode(state.mode); setCategory(state.category); setGenre(state.genre); setArea(state.area); setOpenOnly(state.openOnly); }; sync(); window.addEventListener("popstate", sync); return () => window.removeEventListener("popstate", sync); }, []);
  ...
  const shopKey = `${mode}-${genre}-${area}-${openOnly}`;
