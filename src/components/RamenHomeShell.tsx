...
export function RamenHomeShell({ initialShops, initialTotal, posts }: { initialShops: RamenShop[]; initialTotal: number; posts: WebRamenMentionWithShop[] }) {
  ...
  return <div className="min-h-screen pb-16 md:pb-0">
    <header className="sticky top-0 z-40 border-b border-border bg-white/95 backdrop-blur"><div className="mx-auto flex min-h-[76px] max-w-[1200px] items-center gap-3 px-4 sm:px-6"><button type="button" onClick={() => { changeMode("shops"); window.setTimeout(() => document.querySelector<HTMLInputElement>("input[placeholder*='店名']")?.focus(), 100); }} aria-label="検索を開く" className="grid min-h-11 min-w-11 place-items-center rounded-full text-2xl text-accent hover:bg-accent-light">⌕</button><Link href="/" className="min-w-0 flex-1 text-center" aria-label="らーめんDBのトップへ"><span className="block text-xl font-black tracking-tight text-ink sm:text-2xl">らーめん<span className="text-accent">DB</span></span><span className="hidden text-[10px] font-bold tracking-[.14em] text-text-secondary sm:block">ラーメンの今が見つかるデータベース</span></Link><div className="flex shrink-0 items-center gap-1 rounded-2xl border border-border bg-white p-1" role="tablist" aria-label="表示モード"><button type="button" role="tab" aria-selected={mode === "news"} onClick={() => changeMode("news")} className={`min-h-11 min-w-20 rounded-xl px-4 py-2.5 text-sm font-black sm:min-w-28 sm:text-base ${mode === "news" ? "bg-accent text-white" : "text-text-secondary hover:bg-background-subtle"}`}>ニュース</button><button type="button" role="tab" aria-selected={mode === "shops"} onClick={() => changeMode("shops")} className={`min-h-11 min-w-20 rounded-xl px-4 py-2.5 text-sm font-black sm:min-w-28 sm:text-base ${mode === "shops" ? "bg-accent text-white" : "text-text-secondary hover:bg-background-subtle"}`}>🍜 店舗</button></div></div></header>
