...
export function RamenHomeShell({ initialShops, initialTotal, posts }: { initialShops: RamenShop[]; initialTotal: number; posts: WebRamenMentionWithShop[] }) {
  ...
  const submitSearch = (event: FormEvent) => { event.preventDefault(); const next = queryInput.trim(); setQuery(next); setSearchNonce((value) => value + 1); setMode("shops"); updateUrl({ mode: "shops", q: next || null, soup: soup || null, style: style || null, area: area !== "all" ? area : null }); };
  ...
  const shopKey = `${mode}-${query}-${soup}-${style}-${area}-${openOnly}-${searchNonce}`;
  return <div className="min-h-screen pb-16 md:pb-0">
    <header className="sticky top-0 z-40 border-b border-border bg-white/95 backdrop-blur"><div className="mx-auto flex min-h-[76px] max-w-[1200px] items-center gap-3 px-4 sm:px-6"><button type="button" onClick={() => { changeMode("shops"); window.setTimeout(() => document.querySelector<HTMLInputElement>("input[placeholder*='店名']")?.focus(), 100); }} aria-label="検索を開く" className="grid min-h-11 min-w-11 place-items-center rounded-full text-2xl text-accent hover:bg-accent-light">⌕</button><Link href="/" className="min-w-0 flex-1 text-center" aria-label="らーめんDBのトップへ"><span className="block text-xl font-black tracking-tight text-ink sm:text-2xl">らーめん<span className="text-accent">DB</span></span><span className="hidden text-[10px] font-bold tracking-[.14em] text-text-secondary sm:block">ラーメンの今が見つかるデータベース</span></Link><div className="flex shrink-0 overflow-hidden rounded-xl border border-border bg-background-subtle" role="tablist" aria-label="表示モード"><button type="button" role="tab" aria-selected={mode === "news"} onClick={() => changeMode("news")} className={`min-h-12 min-w-24 px-4 py-2.5 text-sm font-black transition sm:min-w-32 sm:text-base ${mode === "news" ? "bg-[#e84b3c] text-white" : "text-text-secondary hover:bg-white"}`}>ニュース</button><button type="button" role="tab" aria-selected={mode === "shops"} onClick={() => changeMode("shops")} className={`min-h-12 min-w-24 px-4 py-2.5 text-sm font-black transition sm:min-w-32 sm:text-base ${mode === "shops" ? "bg-[#f39c12] text-white" : "text-text-secondary hover:bg-white"}`}>🍜 店舗</button></div></div></header>
