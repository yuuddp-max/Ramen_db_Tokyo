...
const PAGE_SIZE = 10;
type Shop = { place_id: string; name: string; address: string | null; genres?: string[] | null; rating: number | null; user_ratings_total: number | null };
...
export function ShopNameMaintenance() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const response = await fetch(`/api/research/admin/shop-name-maintenance?q=${encodeURIComponent(query)}&nonRamen=${nonRamenOnly ? "1" : "0"}&includeExcluded=${includeExcluded ? "1" : "0"}&limit=${PAGE_SIZE}&offset=${page * PAGE_SIZE}`, { cache: "no-store" });
  ...
  }, [page, query, nonRamenOnly, includeExcluded]);
  ...
  return <section className="panel mt-8 rounded-2xl p-6">
    ...
    <p className="mt-3 text-sm text-stone-400">登録済み店舗の店名や削除状態を管理します。削除すると ramen_shops.is_excluded=true の削除フラグが付き、一覧・検索対象から除外されます。</p>
    ...
    <div className="mt-3 space-y-3">{shops.map((shop) => <article key={shop.place_id} className={`rounded-xl border border-white/10 p-4 ${includeExcluded ? "bg-red-950/20" : "bg-black/20"}`}><div className="flex flex-wrap items-center gap-3"><input type="checkbox" checked={selected.has(shop.place_id)} onChange={(event) => setSelected((current) => { const next = new Set(current); if (event.target.checked) next.add(shop.place_id); else next.delete(shop.place_id); return next; })} aria-label={`${shop.name}を選択`} /><p className="text-sm text-stone-400">{shop.address ?? "住所未登録"}</p><p className="text-xs text-stone-500">Google {shop.rating?.toFixed(1) ?? "–"} / 口コミ {shop.user_ratings_total?.toLocaleString() ?? "–"}件</p>{includeExcluded && <span className="rounded bg-red-400/10 px-2 py-1 text-xs font-bold text-red-300">削除済み</span>}</div><div className="mt-3 flex flex-col gap-2 sm:flex-row"><input value={names[shop.place_id] ?? shop.name} onChange={(event) => setNames((current) => ({ ...current, [shop.place_id]: event.target.value }))} className="min-w-0 flex-1 rounded-lg border border-white/15 bg-black px-3 py-2 font-bold text-white" aria-label={`${shop.name}の店名`} /><button disabled={saving !== null || loading || includeExcluded} onClick={() => void save(shop)} className="rounded-lg bg-emerald-400 px-4 py-2 font-bold text-ink">{saving === shop.place_id ? "保存中…" : "店名を保存"}</button><button disabled={saving !== null || loading || includeExcluded} onClick={() => void remove(shop)} className="rounded-lg border border-red-400/70 px-4 py-2 font-bold text-red-300 hover:bg-red-400/10">削除</button></div></article>)}</div>
