...
export function ShopCard({ shop, index }: { shop: RamenShop; index: number }) {
  return <Link href={`/shops/${shop.id}`} className="panel group block rounded-2xl p-4 transition hover:-translate-y-1 hover:border-gold/60">
    ...
    <div className="mt-4 flex items-center gap-2 text-xs text-stone-400"><span className="rounded bg-white/5 px-2 py-1">{formatPriceLevel(shop.price_level)}</span>{shop.genres?.slice(0, 2).map((genre) => <span className="rounded bg-white/5 px-2 py-1" key={genre}>{genre.replaceAll("_", " ")}</span>)}</div>
  </Link>;
}
