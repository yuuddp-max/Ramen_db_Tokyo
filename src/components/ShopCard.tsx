...
export function ShopCard({ shop, selected = false }: { shop: RamenShop; distanceM?: number; selected?: boolean }) {
  ...
  return <article id={`shop-card-${shop.id}`} className={`group border-b border-border p-5 transition last:border-b-0 hover:bg-background-subtle ${selected ? "bg-accent-light" : ""}`}>
    ...
    <div className="mt-4 grid gap-4 text-sm sm:grid-cols-2"><div><div className="flex flex-wrap items-center gap-x-2 gap-y-1"><p className="text-xs text-text-muted">営業状況</p><p className={`font-bold ${status.open ? "text-success" : status.known ? "text-text-secondary" : "text-text-muted"}`}>{status.label}</p><span className="text-xs text-text-secondary">{hours}</span></div></div><div><div className="flex flex-wrap items-center gap-x-2 gap-y-1"><p className="text-xs text-text-muted">Googleの評価</p><p className="font-bold text-rating">★ {shop.rating?.toFixed(1) ?? "–"}</p><span className="text-xs text-text-secondary">（{shop.user_ratings_total?.toLocaleString() ?? 0}件）</span></div></div></div>
