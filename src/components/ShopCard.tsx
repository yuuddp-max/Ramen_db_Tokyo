...
export function ShopCard({ shop, distanceM, selected = false }: { shop: RamenShop; distanceM?: number; selected?: boolean }) {
  ...
  const distance = distanceM == null ? null : distanceM >= 1000 ? `東京駅から ${(distanceM / 1000).toFixed(1)}km` : `東京駅から ${Math.round(distanceM)}m`;
  return <article id={`shop-card-${shop.id}`} className={`group border-b border-border p-5 transition last:border-b-0 hover:bg-background-subtle ${selected ? "bg-accent-light" : ""}`}>
    ...
    <div className="mt-4 grid gap-4 text-sm sm:grid-cols-2 lg:grid-cols-4"><div><p className="text-xs text-text-muted">営業状況</p><p className={`mt-1 font-bold ${status.open ? "text-success" : status.known ? "text-text-secondary" : "text-text-muted"}`}>{status.label}</p><p className="mt-1 text-xs text-text-secondary">{hours}</p></div><div><p className="text-xs text-text-muted">Googleの評価</p><p className="mt-1 font-bold text-rating">★ {shop.rating?.toFixed(1) ?? "–"} <span className="font-normal text-text-secondary">（{shop.user_ratings_total?.toLocaleString() ?? 0}件）</span></p></div><div><p className="text-xs text-text-muted">推定混雑度</p><p className="mt-1 font-bold text-ink">{estimateCrowd(shop.rating)}<span className="ml-1 text-xs font-normal text-text-muted">（推定）</span></p></div><div><p className="text-xs text-text-muted">距離</p><p className="mt-1 font-bold text-text-secondary">{distance ?? "東京駅を基準"}</p></div></div>
