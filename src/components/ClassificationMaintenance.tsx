...
export function ClassificationMaintenance() {
  ...
  return <section className="panel mt-8 rounded-2xl p-6">
    ...
    <div className="mt-3 space-y-3">{shops.map((shop) => { const selected = values[shop.place_id] ?? { soup: shop.soupCategory ?? "不明", style: shop.styleCategory ?? "不明" }; return <article key={shop.place_id} className="rounded-xl border border-white/10 bg-black/20 p-4"><div className="flex flex-wrap items-start justify-between gap-3"><div><h3 className="font-bold">{shop.name}</h3><p className="mt-1 text-sm text-stone-400">{shop.address ?? "住所未登録"}</p><p className="mt-1 text-xs text-stone-500">Google {shop.rating?.toFixed(1) ?? "–"} / 口コミ {shop.user_ratings_total?.toLocaleString() ?? "–"}件</p></div><span className="rounded bg-white/5 px-2 py-1 text-xs text-stone-400">{shop.classificationStatus ?? "未分類"}</span></div><div className="mt-3 grid gap-3 sm:grid-cols-[1fr_1fr_auto] sm:items-end"><label className="text-sm font-medium text-white">スープ系統<select value={selected.soup} onChange={(event) => setValues((current) => ({ ...current, [shop.place_id]: { ...selected, soup: event.target.value } }))} className="mt-1 block w-full rounded-lg border border-white/15 bg-black px-3 py-2 text-white">{SOUPS.map((value) => <option key={value}>{value}</option>)}</select></label><label className="text-sm font-medium text-white">スタイル<select value={selected.style} onChange={(event) => setValues((current) => ({ ...current, [shop.place_id]: { ...selected, style: event.target.value } }))} className="mt-1 block w-full rounded-lg border border-white/15 bg-black px-3 py-2 text-white">{STYLES.map((value) => <option key={value}>{value}</option>)}</select></label><button disabled={saving === shop.place_id} onClick={() => void save(shop)} className="rounded-lg bg-emerald-400 px-4 py-2 font-bold text-ink">{saving === shop.place_id ? "保存中…" : "修正を保存"}</button></div></article>; })}</div>
