...
export function SearchExperience({ initialShops, initialTotal }: Props) {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      ...
      setSearchError("");
      try {
        const params = new URLSearchParams({ q: query, genre, soup, style, minRating, price, sort, limit: "60", offset: "0" });
  ...
  }, [query, genre, soup, style, minRating, price, sort, favoriteOnly, favoriteIds, recentOnly, recentIds, openOnly]);
  ...
  const loadMore = async () => {
    ...
    setSearchError("");
    try {
      const params = new URLSearchParams({ q: query, genre, soup, style, minRating, price, sort, limit: "60", offset: String(shops.length) });
      if (favoriteOnly) params.set("ids", favoriteIds.join(","));
      if (recentOnly) params.set("ids", recentIds.join(","));
      if (openOnly) params.set("openNow", "true");
  ...
  const clearFilters = () => {
    setQuery(""); setGenre(""); setSoup(""); setStyle(""); setMinRating(""); setPrice(""); setSort("rating"); setFavoriteOnly(false); setRecentOnly(false); setOpenOnly(false);
  };
