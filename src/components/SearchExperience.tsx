...
export function SearchExperience({ initialShops, initialTotal }: Props) {
  ...
  useEffect(() => {
    const timer = setTimeout(async () => {
      setLoading(true);
      const params = new URLSearchParams({ q: query, sort, limit: "60" });
  ...
  const filteredLabel = useMemo(() => query ? `「${query}」の検索結果` : "東京のラーメン店", [query]);
  return <>
    <section className="relative overflow-hidden border-b border-white/10 bg-charcoal"><div className="grain absolute inset-0 opacity-60" /><div className="relative mx-auto max-w-7xl px-5 pb-12 pt-16 sm:px-8 sm:pt-24">
      ...
      <div className="panel mt-8 flex flex-col gap-3 rounded-2xl p-3 sm:flex-row"><label className="flex flex-1 items-center gap-3 rounded-xl bg-black/40 px-4 py-3"><span className="text-gold">⌕</span><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="店名、駅名、エリアで検索" className="w-full bg-transparent text-sm outline-none placeholder:text-stone-600" /></label><select value={sort} onChange={(event) => setSort(event.target.value)} className="rounded-xl bg-white/10 px-4 py-3 text-sm outline-none"><option value="rating">評価順</option><option value="newest">新着順</option></select></div>
    </div></section>
