...
export function SearchExperience({ initialShops, initialTotal }: Props) {
  ...
  }, [query, genre, style, sort, favoriteOnly, favoriteIds]);
  ...
  }, [query, genre, style, favoriteOnly]);
  const hasActiveFilters = Boolean(query || genre || style || sort !== "rating" || favoriteOnly);
  const clearFilters = () => {
    setQuery(""); setGenre(""); setStyle(""); setSort("rating"); setFavoriteOnly(false);
  };
  return <>
    <section className="relative overflow-hidden border-b border-white/10 bg-charcoal"><div className="grain absolute inset-0 opacity-60" /><div className="relative mx-auto max-w-7xl px-5 pb-12 pt-16 sm:px-8 sm:pt-24">
      ...
      <div className="panel mt-8 rounded-2xl p-3"><div className="flex flex-col gap-3 sm:flex-row"><label className="flex flex-1 items-center gap-3 rounded-xl bg-black/40 px-4 py-3"><span className="text-gold">⌕</span><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="店名、駅名、エリアで検索" className="w-full bg-transparent text-sm outline-none placeholder:text-stone-600" /></label><select value={sort} onChange={(event) => setSort(event.target.value as "rating" | "reviews" | "newest")} className="rounded-xl bg-white/10 px-4 py-3 text-sm outline-none"><option value="rating">評価順</option><option value="reviews">口コミ数順</option><option value="newest">新着順</option></select></div><div className="mt-3 flex gap-2 overflow-x-auto pb-1" aria-label="ラーメンジャンル検索">{ramenStyles.map((item) => <button key={item.label} onClick={() => { setGenre(item.genre); setStyle(item.style); }} className={`shrink-0 rounded-full border px-3 py-1.5 text-xs font-bold transition ${genre === item.genre && style === item.style ? "border-gold bg-gold text-ink" : "border-white/10 bg-white/5 text-stone-400 hover:border-gold/60 hover:text-gold"}`}>{item.label}</button>)}</div></div>
    </div></section>
    <section className="mx-auto max-w-7xl px-5 py-8 sm:px-8"><div className="mb-5 flex flex-wrap items-end justify-between gap-4"><div><p className="text-sm text-stone-400">{filteredLabel}</p><p className="mt-1 text-xl font-bold">{total.toLocaleString()} <span className="text-sm font-normal text-stone-500">shops found</span></p></div><div className="flex flex-wrap items-center justify-end gap-2">{hasActiveFilters && <button onClick={clearFilters} className="px-2 py-2 text-xs text-stone-500 hover:text-gold">条件をクリア</button>}<button onClick={() => setFavoriteOnly((current) => !current)} className={`rounded-xl border px-3 py-2 text-sm font-bold transition ${favoriteOnly ? "border-ramen bg-ramen text-white" : "border-white/10 text-stone-400 hover:border-gold hover:text-gold"}`}>♡ お気に入り</button><div className="rounded-xl border border-white/10 p-1"><button onClick={() => setView("list")} className={`rounded-lg px-3 py-2 text-sm ${view === "list" ? "bg-gold text-ink" : "text-stone-400"}`}>一覧</button><button onClick={() => setView("map")} className={`rounded-lg px-3 py-2 text-sm ${view === "map" ? "bg-gold text-ink" : "text-stone-400"}`}>地図</button></div></div></div>
